...
int main()
{
	...
	cout << "Enter the number of paths to connect all colleges  ---> ";
	cin >> number;
	...
	for (int i = 0; i < number; i++)
	{
		cout << "Enter the cost, source and destination college of path " << i << " ---->\n";
		cin >> edges[i].weight;
		cin >> edges[i].source;
    ...
    // for(int i =0;i<number;i++){
    //     cout << "\n\t" << edges[i].weight << "\t" << edges[i].source << "\t" << edges[i].destination;
    // }

	ascending(edges, number);
    // cout << "\n\tShortest path to connect all colleges :\n\tCost\tSource\tDestination";
	...
	cout << "\n\tTotal Cost to connect all colleges = " << cost;
